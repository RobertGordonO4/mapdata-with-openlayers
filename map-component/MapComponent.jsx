...
const MapComponent = () => {
  ...
  const [hoveredAngle, setHoveredAngle] = useState(null); // <-- State for hover angle
  ...
  const displayHoveredAngle = () => {
    ...
    // Always display in degrees for simplicity, add toggle if needed later
  ...
  const removeInteraction = useCallback((interactionRef) => {
    if (interactionRef.current && mapInstanceRef.current) {
      try {
        const interactions = mapInstanceRef.current
          .getInteractions()
          .getArray();
